...
def download_and_extract_ffmpeg(ffmpeg_url, ffmpeg_extract_path):
    """
    Download and extract FFmpeg from the given URL.
    Args:
        ffmpeg_url (str): URL to download the FFmpeg ZIP file.
        ffmpeg_extract_path (str): Absolute path to the directory to extract FFmpeg.
    Returns:
        str or None: Path to the FFmpeg bin directory if successful, None otherwise.
    """
    # Download the zip file to the project root
    ffmpeg_zip_path = os.path.join(PROJECT_ROOT, "ffmpeg.zip")
    ...
    try:
        ...
        if os.path.exists(ffmpeg_zip_path):  # Clean up bad zip
            os.remove(ffmpeg_zip_path)
        return None
    finally:
        # Clean up the zip file after extraction attempt
        if os.path.exists(ffmpeg_zip_path):
            os.remove(ffmpeg_zip_path)

    return validate_ffmpeg_directory(ffmpeg_extract_path)
...
def download_and_install_ffmpeg():
    ...
    if platform.system() == "Windows":
        ...
        # Define the ffmpeg installation directory within the project root
        ffmpeg_installation_dir = os.path.join(PROJECT_ROOT, "ffmpeg")

        # Check if FFmpeg is already downloaded and extracted
        if os.path.exists(ffmpeg_installation_dir):
            ffmpeg_bin = validate_ffmpeg_directory(ffmpeg_installation_dir)
        ...
        # Download and extract FFmpeg to the project root's ffmpeg directory
        ffmpeg_bin = download_and_extract_ffmpeg(ffmpeg_url, ffmpeg_installation_dir)
        if not ffmpeg_bin:
